...
def _generate_request(text: str, model: str, *, num_ctx: int, num_predict: int, think: bool) -> tuple[str, dict]:
    ...
    return "/api/generate", {"model": model, "prompt": prompt, "stream": False, "format": SEMANTIC_OUTPUT_SCHEMA,
                              "think": think, "options": {"num_ctx": num_ctx, "num_predict": num_predict}}


def _chat_template_request(text: str, model: str, *, num_ctx: int, num_predict: int, think: bool) -> tuple[str, dict]:
...
# (see NUEXTRACT_CHAT_TEMPLATE's docstring comment above): an explicit opt-in,
# never a silent default switch.
...
def semantic_extract_batch(records: list[dict], evidence_root: Path, *, artist: str, endpoint: str,
    ...
    write_json(evidence_root / "semantic" / "run_manifest.json", {"artist": artist, "model": model, "context": context,
        ...
        "max_adaptive_depth": max_adaptive_depth, "api_style": api_style, "status": "SUCCESS", "attempts": attempts_manifest, "nodes": nodes_manifest})

...
## Second-check indipendente su "GMV Crawler — Specifica v0.2" (verificato 2026-09-13, repo `GMVrgb76/gmv-core-notion-pipeline`, worktree `bridge-cse_...`)

Correzione di una nota precedente (riga "Gap immagini/.doc in extract()", 2026-08-29): **superata**. `10_API/gmv_evidence_pipeline.py` ha ora `_paddleocr_extract()` (righe ~199+), che shella verso un venv Python 3.11 isolato (`PADDLEOCR_VENV_PYTHON`, documentato in `10_API/GMV_OCR_PADDLEOCR.md`) per PDF scansionati senza layer testo. `OCR_REQUIRED` resta uno stato terminale ma ora ha un percorso di risoluzione reale, non è più solo un'ancora dichiarata. Non fidarsi della nota vecchia: l'AI-002 backlog item potrebbe essere già parzialmente implementato — verificare lo stato backlog separatamente prima di riproporlo come gap aperto.

Fatti verificati per la proposta "GMV Crawler":

- **Nessun componente `gmv-dropbox-import` esiste nel repo**, in nessuna forma (script, pacchetto, sottocartella), a nessun path. "Dropbox" nel codice compare solo come (a) path literal in `gmv_artist_import.py` (`DEFAULT_TEMPLATE`/`DEFAULT_DESTINATION_ROOT` sotto `~/Library/CloudStorage/Dropbox/...`) e (b) nome di parametro (`dropbox_root`) passato a `scan()`/`extract()` in `gmv_evidence_pipeline.py`. Nessuna logica di triage a tre liste (Certi/Da-decidere/Non-toccare) esiste nel repo. Se un documento di progetto cita "gmv-dropbox-import v3" come già validato, va trattato come non verificato/probabilmente esterno o immaginario finché non se ne trova la fonte reale — non dare per buono solo perché un documento precedente lo dichiara.
- **`gmv_run_ledger.py` (root) è generico**: `STAGE_TRANSITIONS`/`RunLedger.append_event(stage=...)` accettano qualunque stringa di stage: non ha stage `extract→adapt→audit→remediation→finalize` hardcoded nel ledger stesso. Quella sequenza specifica (`10_EXTRACT`, `20_ADAPT`, `30_AUDIT`, `40_REMEDIATION_PLAN`, `80_FINALIZE`) è definita da `gmv_pipeline.py` (Area35 QA), il *consumatore* del ledger, non dal ledger. Un nuovo consumatore (il crawler) definirebbe il proprio elenco di stage (es. `10_DISCOVER`, `20_REGISTER`, ... `90_AUDIT`) riusando lo stesso meccanismo generico, senza dover riusare i nomi letterali di Area35 QA.
- **`area35_validator.py` ha `REGOLE = [r_struttura, r_relazioni, r_temporale, r_monade, r_testo, r_duplicati, r_normalizzazione]`** (7 funzioni). "Provenance/fonte" non è una famiglia a sé: la risolvibilità della fonte (`livello_fonte`, verifica campo-fonte verso SUM) vive dentro `r_monade` ("famiglia critica: risolvibilità fonte, disciplina epistemica, gate PUBLIC", SPEC §5/§13/§14). Chi propone di "estendere area35_validator.py" per il crawler deve sapere che il punto d'innesto naturale per regole di provenienza epistemica è `r_monade`, non una nuova funzione parallela chiamata "provenance".
- **Bug di cancellazione silenziosa in `scan()` confermato reale**, `10_API/gmv_evidence_pipeline.py:194`: `if not row["paths"]: del old[fid]` — quando un file content-hash non ha più percorsi residui nello scope scansionato, la riga sparisce da `FILE_INDEX.jsonl` senza transizione di stato né audit trail. Riproducibile leggendo `scan()` per intero (righe ~165-194).
- **`gmv_core/repositories/resources.py` esiste già con un vero registro content-addressed a livello Core**: tabella `resources` con colonne `sha256`, `size_bytes`, `imported_at`, `status`, OID `RES-*`. Questo è DIVERSO e SEPARATO dal `FILE_INDEX.jsonl` file-based usato da `gmv_evidence_pipeline.py` (che è fuori dal Core persistence boundary, vedi nota precedente in questa memoria). **Prima di disegnare un "Registry SQLite schema" nuovo per il crawler (step 3 del piano v0.2), va decisa esplicitamente la relazione con `gmv_core/repositories/resources.py`**: se il crawler introduce un terzo meccanismo di identità content-hash parallelo (oltre a `FILE_INDEX.jsonl` ed a `resources`), è una duplicazione da segnalare, non un dettaglio implementativo.
- **`gmv_core/` ha già un pattern di repository (`gmv_core/repositories/{objects,relations,resources,queue,search,services,identity}.py`)** dietro `ADR_CORE_PERSISTENCE_BOUNDARY.md` (18/21 script in `10_API/` chiamano ancora `sqlite3.connect` direttamente, migrazione dichiarata incrementale "one vertical slice at a time", enforcement del divieto di `sqlite3.connect` diretto fuori da `gmv_core` è deferred ma la direzione è esplicita). Un nuovo componente greenfield come il crawler non ha la scusa del debito storico: qualunque nuova tabella/registry SQLite dovrebbe passare da `gmv_core/repositories/`, non aggiungere un 22° script con `sqlite3.connect` diretto.
- **Collisione di naming pericolosa**: `00_CONFIG/CONSTITUTION_CLI_FEATURE_FREEZE.md` (FROZEN — UNAPPROVED, dal 2026-07-12) copre un `gmv constitution` CLI/`10_API/constitution_service.py` **completamente diverso e non correlato** (leggeva `~/Library/CloudStorage/Dropbox/.../00_CONSTITUTION`, violava il boundary Core→Dropbox). È un congelamento su un file/servizio specifico con quello stesso nome ("Constitution"), non sul concetto generale. Una "Epistemic Ingestion Constitution v0.2" per il crawler è concettualmente estranea a questo freeze, MA: (a) non deve mai riusare/reincarnare `10_API/constitution_service.py`, (b) non deve mai leggere Dropbox direttamente da un path Core, (c) è raccomandabile evitare il termine nudo "constitution" come nome di subcommand CLI per non creare confusione con la feature congelata — chiamarla esplicitamente "epistemic ingestion rules"/"ingestion policy" nel nome tecnico (file, comando), riservando "Constitution" al solo titolo del documento di governance.
- **`00_CONFIG/GMV_ENGINE_DECISION_AUTOMATION_FREEZE.md`** ha un principio riusabile per gli stage a valle del crawler (RECONCILE, VALIDATE EPISTEMICALLY): cita un vincolo architetturale preesistente "nessun componente può bypassare i gate Reasoning/Decision/autonomous-workflow" — coerente con la fase attiva Core Integrity (Reasoning/Decision sono target futuri, non perimetro attivo). Qualunque step del crawler che "decide" (validazione epistemica, riconciliazione) deve restare rule-based/deterministico e gated da revisione umana, mai un motore decisionale autonomo, e mai schedulato come LaunchAgent senza Service OID/registrazione (stesso precedente già in memoria per l'interfaccia di review).
- **`ADR_DB004_APPEND_ONLY_EVENTS.md`**: Events è l'unica storia canonica, append-only, correzioni = nuovi Event mai update/delete. Lo stage AUDIT del crawler e qualunque "correzione" di un ATOM già materializzato deve riusare questo stesso meccanismo (Events Core, non un log parallelo), coerente col pattern già osservato per `gmv_evidence_ledger.py` ("non crea un secondo ledger").

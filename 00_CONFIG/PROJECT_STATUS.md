...
- **Last Completed Task:** GMV V2 Sprint 003 (Data/Database) — third bounded task: migrated `10_API/service_service.py` (list/runs/show) behind a new `gmv_core/repositories/services.py` repository and `gmv_core/database.connect()`, the same pattern established for `object_service.py`. The script no longer imports `sqlite3` or calls `sqlite3.connect` directly. Behavior-preserving, no schema change, no other service migrated. GMV Core Development itself remains not yet opened as a distinct phase in this ledger; Sprint 003 is tracked as its own GMV V2 program item (see Timeline entry below).
- **Next Approved Task:** None formally approved. Which `10_API` service (of the 16 remaining direct-`sqlite3.connect` callers) is migrated next requires explicit Project Owner selection, as with this task and the prior `object_service.py` slice — the ADR does not prescribe an order. Sprint 003's later execution-sequence steps (JSON/Object identity reconciliation under `MAIN-011`, Timeline-to-Events migration under `DB-005`, and further steps per `GMV_V2_EXECUTION_ROADMAP.md`) are also not yet authorized. GMV Core Development (this ledger's own phase) has not been opened by any specific task.
- **Last Commit:** this task's commit (see `git log`) — "feat: move service_service.py behind the Core persistence boundary".

...
ENTITY_REGISTRY_VERSION = 10
ENTITY_REGISTRY_RESOURCE = "migration_sql/010_entity_registry.sql"
CURRENT_SCHEMA_VERSION = OID_TYPE_CONSISTENCY_VERSION
# CRAWLER_SOURCE_REGISTRY_VERSION and ENTITY_REGISTRY_VERSION are
# intentionally not CURRENT_SCHEMA_VERSION and not in
# SUPPORTED_SCHEMA_VERSIONS: the crawler subsystem that owns these tables
# does not exist yet. They are reachable only via an explicit
# target_version, exactly like versions 1-7 are today -- no caller of
# migrate(db) without an explicit target is affected by their existence.
    ...
    *,
    ...
    try:
        with connect_path(target) as connection:
            ...
            if (
                target_version >= ENTITY_REGISTRY_VERSION
                and current_version == CRAWLER_SOURCE_REGISTRY_VERSION
            ):
                _apply_migration(
                    connection,
                    target=target,
                    version=ENTITY_REGISTRY_VERSION,
                    resource=ENTITY_REGISTRY_RESOURCE,
                )
                current_version = ENTITY_REGISTRY_VERSION
